...
class ExcelToolApp:
    def __init__(self, root):
        ...
        tk.Label(self.frame_stage3, text="Search Type:").grid(row=4, column=0, sticky="e", pady=5)
        ...
        self.search_button.grid(row=3, column=2, padx=5, pady=5, rowspan=2, sticky="n")

        tk.Label(self.frame_stage3, text="Search Results:").grid(row=5, column=0, sticky="nw", pady=5)
        self.results_table_frame = tk.Frame(self.frame_stage3, bd=2, relief="solid")
        self.results_table_frame.grid(row=5, column=1, columnspan=2, sticky="nsew", pady=5)
    ...
    # --- Stage 3 methods (custom grid with borders and blank for nan, search type) ---
    ...
    def perform_search(self):
        ...
        try:
            ...
            if not results_df.empty:
                self.display_results_in_grid(results_df)
            else:
    ...
    def display_results_in_grid(self, df):
        # Display headers
        ...
        for i, (_, row) in enumerate(df.iterrows(), start=1):
            for j, val in enumerate(row):
                # Show blank if value is nan/None
                cell_value = "" if pd.isna(val) or str(val).lower() == "nan" else val
                cell = tk.Label(self.results_inner_frame, text=cell_value, relief="solid", borderwidth=1, bg="white", font=("Arial", 10), padx=5, pady=2)
